...
describe Order, 'validations' do
  ...
  context "limit order" do
    it "should make sure price is present" do
      order = Order.new(currency: 'btcjpy', price: nil, ord_type: 'limit')
      order.should_not be_valid
      order.errors[:price].should == ["is not a number"]
    end

    it "should make sure price is greater than zero" do
      order = Order.new(currency: 'btcjpy', price: '0.0'.to_d, ord_type: 'limit')
      order.should_not be_valid
  ...
  context "market order" do
    it "should make sure price is not present" do
      order = Order.new(currency: 'btcjpy', price: '0.0'.to_d, ord_type: 'market')
      order.should_not be_valid
...
describe Order, "#fix_number_precision" do
  let(:order_bid) { create(:order_bid, currency: 'btcjpy', price: '12.326'.to_d, volume: '123.123456789') }
  let(:order_ask) { create(:order_ask, currency: 'btcjpy', price: '12.326'.to_d, volume: '123.123456789') }
...
describe Order, "related accounts" do
  ...
  context OrderAsk do
    it "should hold btc and expect jpy" do
      ask = create(:order_ask, member: alice)
      ask.hold_account.should == alice.get_account(:btc)
      ask.expect_account.should == alice.get_account(:jpy)
    end
  end

  context OrderBid do
    it "should hold jpy and expect btc" do
      bid = create(:order_bid, member: bob)
      bid.hold_account.should == bob.get_account(:jpy)
...
describe Order, "#avg_price" do
  ...
  it "should calculate average price of bid order" do
    OrderBid.new(currency: 'btcjpy', locked: '10.0', origin_locked: '20.0', volume: '1.0', origin_volume: '3.0', funds_received: '2.0').avg_price.should == '5'.to_d
  end

  it "should calculate average price of ask order" do
    OrderAsk.new(currency: 'btcjpy', locked: '1.0', origin_locked: '2.0', volume: '1.0', origin_volume: '2.0', funds_received: '10.0').avg_price.should == '10'.to_d
  end
end
