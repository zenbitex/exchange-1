...
describe Worker::Matching do
  ...
  context "partial match" do
    ...
    it "should started engine" do
      subject.engines['btcjpy'].mode.should == :run
    end
    ...
    it "should match part of existing order" do
      ...
      AMQPQueue.expects(:enqueue)
        .with(:slave_book, {action: 'update', order: {id: existing.id, timestamp: existing.at, type: :ask, volume: '2.0'.to_d, price: existing.price, market: 'btcjpy', ord_type: 'limit'}}, anything)
      AMQPQueue.expects(:enqueue)
  ...
  context "complex partial match" do
    ...
    let!(:orderbook) { Matching::OrderBookManager.new('btcjpy', broadcast: false) }
  ...
  context "dryrun" do
    ...
    context "very old orders matched" do
      ...
      it "should not start engine" do
        subject.engines['btcjpy'].mode.should == :dryrun
        subject.engines['btcjpy'].queue.should have(1).trade
      end
    end
