...
describe APIv2::Deposits do
  ...
  describe "GET /api/v2/deposits" do
    ...
    before do
      ...
      create(:deposit, member: member, currency: 'jpy')
      create(:deposit, member: member, currency: 'jpy', txid: 1, amount: 520)
      create(:deposit, member: member, currency: 'btc', created_at: 2.day.ago,  txid: 'test', amount: 111)
      create(:deposit, member: other_member, currency: 'jpy', txid: 10)
    end
    ...
    it "deposits currency jpy" do
      signed_get '/api/v2/deposits', params: {currency: 'jpy'}, token: token
      result = JSON.parse(response.body)
      result.should have(2).deposits
      result.all? {|d| d['currency'] == 'jpy' }.should be_true
    end
