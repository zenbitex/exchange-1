...
describe APIv2::Orders do
  ...
  describe "GET /api/v2/orders" do
    before do
      create(:order_bid, currency: 'btcjpy', price: '11'.to_d, volume: '123.123456789', member: member)
      create(:order_bid, currency: 'btcjpy', price: '12'.to_d, volume: '123.123456789', member: member, state: Order::CANCEL)
      create(:order_ask, currency: 'btcjpy', price: '13'.to_d, volume: '123.123456789', member: member)
      create(:order_ask, currency: 'btcjpy', price: '14'.to_d, volume: '123.123456789', member: member, state: Order::DONE)
    end

    it "should require authentication" do
      get "/api/v2/orders", market: 'btcjpy'
    ...
    it "should validate state param" do
      signed_get '/api/v2/orders', params: {market: 'btcjpy', state: 'test'}, token: token
      response.code.should == '400'
    ...
    it "should return active orders by default" do
      signed_get '/api/v2/orders', params: {market: 'btcjpy'}, token: token
      response.should be_success
      JSON.parse(response.body).size.should == 2
    end

    it "should return complete orders" do
      signed_get '/api/v2/orders', params: {market: 'btcjpy', state: Order::DONE}, token: token
      response.should be_success
      JSON.parse(response.body).first['state'].should == Order::DONE
    end

    it "should return paginated orders" do
      signed_get '/api/v2/orders', params: {market: 'btcjpy', limit: 1, page: 1}, token: token
      response.should be_success
      JSON.parse(response.body).first['price'].should == '11.0'

      signed_get '/api/v2/orders', params: {market: 'btcjpy', limit: 1, page: 2}, token: token
      response.should be_success
      JSON.parse(response.body).first['price'].should == '13.0'
    end

    it "should sort orders" do
      signed_get '/api/v2/orders', params: {market: 'btcjpy', order_by: 'asc'}, token: token
      response.should be_success
      orders = JSON.parse(response.body)
      orders[0]['id'].should < orders[1]['id']

      signed_get '/api/v2/orders', params: {market: 'btcjpy', order_by: 'desc'}, token: token
      response.should be_success
      orders = JSON.parse(response.body)
      orders[0]['id'].should > orders[1]['id']
    end
  ...
  describe "GET /api/v2/order" do
    let(:order)  { create(:order_bid, currency: 'btcjpy', price: '12.326'.to_d, volume: '3.14', origin_volume: '12.13', member: member, trades_count: 1) }
  ...
  describe "POST /api/v2/orders/multi" do
    before do
      ...
      member.get_account(:jpy).update_attributes(balance: 100000)
    end
  ...
  describe "POST /api/v2/orders" do
    it "should create a sell order" do
      ...
      expect {
        signed_post '/api/v2/orders', token: token, params: {market: 'btcjpy', side: 'sell', volume: '12.13', price: '2014'}
        response.should be_success
    ...
    it "should create a buy order" do
      ...
      expect {
        signed_post '/api/v2/orders', token: token, params: {market: 'btcjpy', side: 'buy', volume: '12.13', price: '2014'}
        response.should be_success
    ...
    it "should return cannot lock funds error" do
      expect {
        signed_post '/api/v2/orders', params: {market: 'btcjpy', side: 'sell', volume: '12.13', price: '2014'}
        response.code.should == '400'
    ...
    it "should give a number as volume parameter" do
      signed_post '/api/v2/orders', params: {market: 'btcjpy', side: 'sell', volume: 'test', price: '2014'}
      response.code.should == '400'
    ...
    it "should give a number as price parameter" do
      signed_post '/api/v2/orders', params: {market: 'btcjpy', side: 'sell', volume: '12.13', price: 'test'}
      response.code.should == '400'
      response.body.should == '{"error":{"code":2002,"message":"Failed to create order. Reason: Validation failed: Price must be greater than 0"}}'
    end
  end
  ...
  describe "POST /api/v2/order/delete" do
    let!(:order)  { create(:order_bid, currency: 'btcjpy', price: '12.326'.to_d, volume: '3.14', origin_volume: '12.13', locked: '20.1082', origin_locked: '38.0882', member: member) }

    context "succesful" do
      before do
        member.get_account(:jpy).update_attributes(locked: order.price*order.volume)
      end
  ...
  describe "POST /api/v2/orders/clear" do
    ...
    before do
      create(:order_ask, currency: 'btcjpy', price: '12.326', volume: '3.14', origin_volume: '12.13', member: member)
      create(:order_bid, currency: 'btcjpy', price: '12.326', volume: '3.14', origin_volume: '12.13', member: member)

      member.get_account(:btc).update_attributes(locked: '5')
      member.get_account(:jpy).update_attributes(locked: '50')
    end
