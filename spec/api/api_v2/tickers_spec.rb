...
describe APIv2::Tickers do
  ...
  describe "GET /api/v2/tickers" do
    it "returns ticker of all markets" do
      ...
      JSON.parse(response.body)['btcjpy']['at'].should_not be_nil
      JSON.parse(response.body)['btcjpy']['ticker'].should == {"buy"=>"0.0", "sell"=>"0.0", "low"=>"0.0", "high"=>"0.0", "last"=>"0.0", "vol"=>"0.0"}
    end
  end

  describe "GET /api/v2/tickers/:market" do
    it "should return market tickers" do
      get "/api/v2/tickers/btcjpy"
      response.should be_success
