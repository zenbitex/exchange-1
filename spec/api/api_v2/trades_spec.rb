...
describe APIv2::Trades do
  ...
  let(:member) do
    create(:verified_member).tap {|m|
      ...
      m.get_account(:jpy).update_attributes(balance: 2014.47, locked: 0)
    }
  end
  let(:token)  { create(:api_token, member: member) }

  let(:ask) { create(:order_ask, currency: 'btcjpy', price: '12.326'.to_d, volume: '123.123456789', member: member) }
  let(:bid) { create(:order_bid, currency: 'btcjpy', price: '12.326'.to_d, volume: '123.123456789', member: member) }
  ...
  describe 'GET /api/v2/trades' do
    ...
    it "should return trades before timestamp" do
      ...
      get '/api/v2/trades', market: 'btcjpy', timestamp: 8.hours.ago.to_i, limit: 1
      response.should be_success
    ...
    it "should return trades between id range" do
      ...
      get '/api/v2/trades', market: 'btcjpy', from: ask_trade.id, to: another.id
      response.should be_success
      json = JSON.parse(response.body)
      json.should have(1).trade
      json.first['id'].should == bid_trade.id
    ...
    it "should get trades by from and limit" do
      another = create(:trade, bid: bid, created_at: 6.hours.ago)
      get '/api/v2/trades', market: 'btcjpy', from: ask_trade.id, limit: 1, order_by: 'asc'
      response.should be_success
      JSON.parse(response.body).first['id'].should == bid_trade.id
    end
  end
  ...
  describe 'GET /api/v2/trades/my' do
    it "should require authentication" do
      get '/api/v2/trades/my', market: 'btcjpy', access_key: 'test', tonce: time_to_milliseconds, signature: 'test'
    ...
    it "should return all my recent trades" do
      signed_get '/api/v2/trades/my', params: {market: 'btcjpy'}, token: token
      response.should be_success
    ...
    it "should return 1 trade" do
      signed_get '/api/v2/trades/my', params: {market: 'btcjpy', limit: 1}, token: token
      response.should be_success
      JSON.parse(response.body).should have(1).trade
    end

    it "should return trades before timestamp" do
      signed_get '/api/v2/trades/my', params: {market: 'btcjpy', timestamp: 30.hours.ago.to_i}, token: token
      response.should be_success
      JSON.parse(response.body).should have(1).trade
    end

    it "should return limit out of range error" do
      signed_get '/api/v2/trades/my', params: {market: 'btcjpy', limit: 1024}, token: token
