...
def key(market, period = 1)
  "exchangepro:#{market}:k:#{period}"
end

def last_ts(market, period = 1)
  latest = @r.lindex key(market, period), -1 # -1: last element
  latest && Time.at(JSON.parse(latest)[0]) #if it left-hand is false or nil, -> return this, the right-hand argument isn't evaluated.
end
...
def append_point(market, period, ts)
  ...
  if period == 1
    ...
    if point = @r.lindex(key(market, period), -1441)
      Rails.cache.write "exchangepro:#{market}:ticker:open", JSON.parse(point)[4]
    end
  end
end
