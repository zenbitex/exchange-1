namespace :replay do
  ...
  task account: :environment do
    ...
    players = Member.find_all_by_email(['foo@exchangepro.dev','bar@exchangepro.dev']).collect do |m|
      previous = data.find{|hash| hash['id'] == m.id }
      array = previous ? previous['balance_array'] : []
      {id: m.id, balance_array: replay(m, array)}
    end

    students = Member.find_all_by_email(['foo@exchangepro.dev','bar@exchangepro.dev']).collect do |m|
      previous = data.find{|hash| hash['id'] == m.id }
      array = previous ? previous['balance_array'] : []
  ...
  def replay(m, bln_arr)
    ...
    (bln_arr.size...arr_size).each do |i|
      price = Trade.with_currency('btcjpy').where('created_at <= ?', Time.at(start.to_i + period * i)).last.price
      bln_arr[i] = (balances['jpy'][i] + (balances['btc'][i] - 1) * price).to_f.round(2).to_s
    end
  ...
  def replay_student(m, bln_arr)
    ...
    (bln_arr.size...arr_size).each do |i|
      price = Trade.with_currency('btcjpy').where('created_at <= ?', Time.at(start.to_i + period * i)).last.price
      bln_arr[i] = ((balances['jpy'][i] + (balances['btc'][i] - 0.1) * price) * 10).to_f.round(2).to_s
    end
