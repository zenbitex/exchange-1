module ApplicationHelper
  ...
  def language_current
    I18n.locale
  end

  def get_menu_current
    controller = params[:controller]
    path_current = ""
    case controller
    when "accounts"
      path_current = accounts_path
    when "private/sendcoin"
      path_current = sendcoin_index_path
    when "private/bank_accounts"
      path_current = new_bank_accounts_path
    when "private/history"
      if params[:action] == "signup_history"
        path_current = signup_history_path
      else 
        path_current = order_history_path
      end
    when "securities"
      path_current = security_path
    when "faqs"
      path_current = faqs_path
    when "private/affiliates"
      path_current = affiliates_path
    when "private/api_payment"
      path_current = payment_system_path
    else 
      path_current = id_document_path
    end
    path_current
  end
  ...
  def yesno(val)
    if val
      ...
      content_tag(:span, 'NG', class: 'label label-danger')
    end
  end
  ...
  def state(val)
    if val == "unverified"
      content_tag(:span, '拒否した', class: 'label label-danger')
    elsif val == "verifying"
      content_tag(:span, '確認中', class: 'label label-warning')
    else
  ...
  def sortable(column, title = nil)
    ...
    link_to title, {:sort => column, :direction => direction, :search => {:content_search => @search_content}}, {:class => css_class}
  end
