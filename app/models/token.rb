class Token < ActiveRecord::Base
  ...
  before_validation :generate_token, on: :create, :unless => :mobile_active?
  before_validation :generate_token_mobile, on: :create, :if => :mobile_active?
  ...
  def generate_token
    ...
    def generate_token_mobile
    self.token = '%04d' % SecureRandom.random_number(10000)
    self.expire_at = 2.minutes.from_now
  end

  def mobile_active?
    self.type == 'Token::ActivationEmailMobile' || self.type == 'Token::ResetPasswordMobile'
  end
