class Account < ActiveRecord::Base
  ...
  CLOUD_SAFE_BUY = :cloud_safe_buy
  CLOUD_SAFE_SELL = :cloud_safe_sell
  SEND_COIN = :send_coin
  ...
  WITHDRAW = :withdraw
  ARB = :arb
  WITHDRAW_TAO = :withdraw_tao
  TAOCOIN_EXCHANGE = :taocoin_exchange
  BONUS = :bonus
  BONUS_AFFILIATE = :bonus_affiliate
  CREDIT_CARD = :credit_card
  INVITE_BONUS = :invite_bonus
  SEND_PROFIT = :send_profit
  COIN_TRADE_BID = :coin_trade_bid
  COIN_TRADE_ASK = :coin_trade_ask
  HARD_FORK_BCH = :hard_fork_bch
  ZERO = 0.to_d
  KBR_FEE = :kbr_fee
  WITHDRAW_KBR = :withdraw_kbr
  ...
  scope :not_fiat, -> { where("currency in (?) AND currency != ?", Currency.ids, 1)}

  scope :by_currencies, ->ids{ where currency: ids }
