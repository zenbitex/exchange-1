class OrderBid < Order
  ...
  def avg_price
    return ::Trade::ZERO if funds_received.zero? || config.nil?
  ...
  def compute_locked
    ...
    when 'limit'
      total = price*volume
      real_fee = (total * fee).round(fee_fixed)
      total + real_fee
    when 'market'
  ...
  def ensure_account_balance
    if self.origin_volume * self.price > member.accounts.find_by_currency(self.bid_value).balance
      errors.add(:amount, I18n.t("private.markets.show.insufficient_funds", currency: self.bid.upcase))
    end
  end
end
