...
#
# ID of market is always in the form "#{B}#{A}". For example, in 'btcjpy'
# market, the commodity pair is `{btc, jpy}`. Sellers sell out _btc_ for
# _jpy_, buyers buy in _btc_ with _jpy_. _btc_ is the `base_unit`, while
# _jpy_ is the `quote_unit`.
...
class Market < ActiveYamlBase
  ...
  def unit_info
    {name: name, base_unit: base_unit, quote_unit: quote_unit, bid: bid, ask: ask}
  end

  def self.price_precision(pair)
    market = where(id: pair).first
