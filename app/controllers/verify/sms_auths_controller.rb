  ...
  class SmsAuthsController < ApplicationController
    ...
    def send_code_phase
      ...
      respond_to do |format|
        if @sms_auth.phone_number.zenkaku?
          text = I18n.t('.activerecord.attributes.two_factor/sms.phone_error')
          format.any { render status: :bad_number, text: {text: text}.to_json }
        else
          if @sms_auth.country.blank?
            text = I18n.t('.activerecord.attributes.two_factor/sms.country_error')
            format.any { render status: :bad_request, text: {text: text}.to_json }
          elsif @sms_auth.valid?
            @sms_auth.send_otp
    ...
    def verify_code_phase
      ...
      respond_to do |format|
        if @sms_auth.verify?
          ...
          # format.any { redirect_to settings_path, notice: t('.notice.already_activated') }
